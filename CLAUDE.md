...
| Backfill YouTube descriptions (dry run) | `./fix-youtube-descriptions.sh --dry-run` |
| Backfill YouTube descriptions (live) | `./fix-youtube-descriptions.sh --confirm` |
| Recover a missing archive file | `python -c "from main import archive_specific_date; archive_specific_date('YYYY-MM-DD')"` |
...
## YouTube Description System

Every daily digest video's YouTube description is generated by `build_youtube_description()` in `main.py`. It renders the same per-fact format shown in the video's lower-third overlay, with inline source attribution and compact `accuracy|bias` scores separated by a middle dot. Corrections to past facts propagate into the affected video's description via `propagate_correction_to_youtube()`. A video cannot change after upload; its description can.

### Key functions (all in main.py)

| Function | Purpose |
|---|---|
| `build_youtube_description(date, facts)` | Single source of truth for description voice. Used by the live upload path, the one-shot backfill, and the corrections propagation hook. Truncates at 4800 characters with an archive link pointer. |
| `parse_daily_archive(date)` | Reads `data/{date}.txt` (local first) or `docs/archive/YYYY/{date}.txt.gz` (gz fallback) and returns fact dicts in the shape `build_youtube_description` expects. The local-first fallback handles dates whose gz archive has not been written yet. |
| `get_compact_scores_for_source_name(name)` | Bridges source names (stored in archive files) to source IDs (keyed in `CONFIG["sources"]`). Uses `get_source_id_by_name` for case-insensitive lookup, returns `("?", "?")` for unknown names. |
| `load_digest_video_ids_from_feed()` | Parses `docs/feed.xml` and returns `{date: video_id}` by matching `<guid>digest-YYYY-MM-DD</guid>` against `<link>https://youtube.com/watch?v=XXX</link>`. The ongoing source of truth for the backfill orchestrator and corrections propagation. |
| `enumerate_channel_digest_videos()` | YouTube API fallback source of truth. Queries the channel's uploads playlist and matches videos by `[DAILY DIGEST] YYYY-MM-DD` title pattern. Used once by `migrate_feed_xml_digest_entries` to seed feed.xml; not needed at runtime. |
| `update_youtube_description(video_id, description)` | Fetch-then-modify wrapper around `videos.update()`. Fetches the existing snippet via `videos.list`, mutates only the description field, sends the full snippet back. Preserves title, tags, and categoryId. Decorated with `@retry_with_backoff(max_retries=3, base_delay=30.0)`. |
| `backfill_youtube_descriptions(dry_run)` | One-shot orchestrator. CLI: `python main.py --backfill-youtube-descriptions[-dry-run]`, or via `./fix-youtube-descriptions.sh`. Idempotent via `data/youtube_description_backfill.json` marker file keyed by video_id. |
| `propagate_correction_to_youtube(date)` | Called from `add_correction_to_rss` after RSS/archive updates complete. Rebuilds the description from the current archive state and pushes it. Same-day corrections produce a byte-identical re-render until `mark_corrected_stories_in_log` runs at the next midnight sweep. |
| `archive_specific_date(date)` | Recovery helper. Gzips a raw `data/{date}.txt` into `docs/archive/YYYY/{date}.txt.gz` and pushes it. Non-destructive: does NOT delete the local raw log. Used to heal archive gaps caused by outages during the normal `archive_daily_log` sweep. |
| `migrate_feed_xml_digest_entries()` | One-time migration. Uses the YouTube API to look up historical digest video_ids and calls `add_digest_to_feed` (with its healed update branch) to add any missing `<link>`/`jtf:type`/`jtf:archive` elements to historical `feed.xml` entries. Idempotent — safe to re-run. |

### Gotchas

- `feed.xml` digest items written before the `add_digest_to_feed` fix lacked `<link>`, `jtf:type`, and `jtf:archive` elements because the update branch only touched an existing `<link>` and silently skipped adding missing elements. The fix now heals all three on both the update path and the new-item path. Historical entries are healed by running `migrate_feed_xml_digest_entries` once.
- The YouTube description format lives in ONE place: `build_youtube_description()`. Do not hardcode descriptions anywhere else in main.py. If the voice changes, change that function.
- YouTube descriptions are capped at 5000 characters; `build_youtube_description()` truncates at 4800 (safety margin) at a fact boundary and inserts an archive link pointer.
- `videos.update()` on the YouTube Data API REPLACES the entire snippet. `update_youtube_description` uses fetch-then-modify to preserve title, tags, and categoryId. Any future code that calls `videos.update()` directly must do the same.
- The backfill marker file `data/youtube_description_backfill.json` is keyed by video_id, not date, so a future correction that replaces a digest video with a new video_id is NOT skipped as "already done". Deleting the marker forces a full re-backfill on next run.
- Corrections MVP: `propagate_correction_to_youtube` re-renders from the current archive state. Inline `CORRECTION` sub-bullets showing original + corrected text side-by-side require extending the archive format to store both. Deferred to a future sprint.
