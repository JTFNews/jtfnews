...
**Purpose:** Extract source data from `config.json` and write to `gh-pages-dist/sources.json` for public consumption.

**Implementation:**

```python
def export_sources_json():
    """Export source ratings and ownership data to gh-pages-dist for public transparency pages."""
    try:
        ...
        # Write to gh-pages-dist
        gh_pages_dir = BASE_DIR / "gh-pages-dist"
        if gh_pages_dir.exists():
            sources_file = gh_pages_dir / "sources.json"
            with open(sources_file, 'w') as f:
                json.dump(export_data, f, indent=2)
            log_event("SOURCES_EXPORT", f"Exported {len(public_sources)} sources to gh-pages-dist")
...
**New file needed:** `gh-pages-dist/archive/index.json`
...
<footer class="page-footer">
    ...
    <p class="footer-links">
        ...
        <a href="https://github.com/larryseyer/JTFNews">GitHub</a> ·
...
7. Verify live site at https://larryseyer.github.io/jtfnews/
...
### Files to Create
- [ ] `gh-pages-dist/archive.html`
- [ ] `gh-pages-dist/sources.html`
- [ ] `gh-pages-dist/corrections.html`
- [ ] `gh-pages-dist/archive/index.json` (generated by main.py)
- [ ] `gh-pages-dist/sources.json` (generated by main.py)

### Files to Modify
- [ ] `main.py` - Add `export_sources_json()` and `update_archive_index()`
- [ ] `gh-pages-dist/index.html` - Update navigation

### External Dependencies
- [ ] pako.js (for archive.html gzip decompression)
  - CDN: `https://cdnjs.cloudflare.com/ajax/libs/pako/2.1.0/pako.min.js`
  - Or download and host locally in `gh-pages-dist/js/pako.min.js`
